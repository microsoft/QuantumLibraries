...
namespace Microsoft.Quantum.Canon {
    ...
    function ComplexMathIsCorrect () : Unit {
        let complexCases = [(0.123, 0.321), (0.123, -0.321), (-0.123, 0.321), (-0.123, -0.321)];

        for (complexRe, complexIm) in complexCases {
    ...
    function PNormIsCorrect () : Unit {
        ...
        for (p, array, pNormExpected) in testCases {
            NearEqualityFactD(PNorm(p, array), pNormExpected);

            // if PNorm fails, PNormalized will fail.
            let arrayNormalized = PNormalized(p, array);

            for (expected, actual) in Zipped(array, arrayNormalized) {
                NearEqualityFactD(expected / pNormExpected, actual);
            }
        }
    }
