...
let ModularReduction (qs:Qubits) = 
    let n = (qs.Length-4)/6
    let ms  = slice qs [(3*n+3)..(4*n+3)]
    let acc = slice qs [(2*n+1)..(3*n+2)]
...
let MontgomeryMultiplier (verbose:bool) (qs:Qubits) = 
    ...
    let b = Circuit.Compile ModularReduction qs
...
let RunMultiplier (name:string) (n:int) (s1:bigint) (s2:bigint) (m:bigint) (verbosity:string) = 
    ...
    let MontgomeryMultiplierCircuit = MUL qs 
    if verbose1 then 
...
//    
    let finalState = MyCircuitSimulate MontgomeryMultiplierCircuit initialState
    if verbose2 then 
...
let __RunCheckAllInputsMultiplierTest (n:int) (p:int) = 
    let P = bigint p
    let L = [ for i in 0I..(P-1I) do yield [ for j in 0I..(P-1I) do yield (RunMultiplierTest n i j P "default") ] ]
            |> List.concat
    List.fold (*) 1 L 
    |> function 
