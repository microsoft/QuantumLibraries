...
namespace Microsoft.Quantum.Samples.SimpleIsing {
    ...
    // FIXME: This is not currently in standard.qb. Since we aren't using
    //        the canon, we need to include a stub here.
    ...
    //
    //     H(s) ? s [J S'?? Z? Z? + hz S? Z?] + (1 - s) + hx S? X?
    //
    // where the primed summation S' is taken only over nearest-neighbors,
    // and where s is a parameter in the interval [0, 1] that controls the
    // sweep to the final Hamiltonian.

    // FIXME: check the rotation angle matches the conventions set by
    //        Rz and Exp!

    // We do so by first defining an operation which will rotate a pair of
    // qubits (q1, q2) by an angle f around ZZ,
    //
    //    U(f) = e^{i f Z1 Z2}.
    //
    ...
    ///
    /// # Output
    /// The results of Z measusrements on each qubit.
    ///
    /// # Remarks
        ...
        body {
            ...
            using (qs = Qubit[nSites]) {
                ...
                for (idxIter in 0..nSteps - 1) {
                    ...
                    let sweepParameter = Float(idxIter) / Float(nSites);
